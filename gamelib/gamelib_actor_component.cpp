...
namespace GameLib {
	...
	void DainNickJosephWorldCollidingActorComponent::handleCollisionWorld(Actor& actor, World& world) {
		...
		for (float x = floor(actor.position.x); x < ceil(actor.position.x + actor.size.x); x += subTileSize) {
			if (world.getTilef(x, floor(actor.position.y)).flags == 1) {
				if (world.getTilef(x, floor(actor.position.y) + subTileSize).flags == 0)
					actor.position.y -= actor.dt * actor.speed * actor.velocity.y;
			}
			if (world.getTilef(x, ceil(actor.position.y + actor.size.y) - subTileSize).flags == 1) {
				if (world.getTilef(x, ceil(actor.position.y + actor.size.y) - 2 * subTileSize).flags == 0)
					actor.position.y -= actor.dt * actor.speed * actor.velocity.y;
			}
		}
		for (float y = floor(actor.position.y); y < ceil(actor.position.y + actor.size.y); y += subTileSize) {
			if (world.getTilef(floor(actor.position.x), y).flags == 1) {
				if (world.getTilef(floor(actor.position.x) + subTileSize, y).flags == 0)
					actor.position.x -= actor.dt * actor.speed * actor.velocity.x;
			}
			if (world.getTilef(ceil(actor.position.x + actor.size.x) - subTileSize, y).flags == 1) {
				if (world.getTilef(ceil(actor.position.x + actor.size.x) - 2 * subTileSize, y).flags == 0)
					actor.position.x -= actor.dt * actor.speed * actor.velocity.x;
			}
		}
	}
