...
void testSprites(GameLib::Context& context, int spriteCount, int& spritesDrawn, SDL_Texture* testPNG,
				 SDL_Texture* testJPG);

std::vector<GameLib::Actor*> actorPool;

GameLib::Actor* MakeActor(float x, float y, float speed, int spriteId, GameLib::InputComponent* ic,
						  GameLib::ActorComponent* ac, GameLib::PhysicsComponent* pc, GameLib::GraphicsComponent* gc) {
	GameLib::Actor* actor = new GameLib::Actor(ic, ac, pc, gc);
	actor->position.x = x;
	actor->position.y = y;
	actor->speed = speed;
...
int main(int argc, char** argv) {
	...
	auto NewDungeonActor = []() { return new GameLib::DungeonActorComponent(); };
	auto NewInput = []() { return new GameLib::SimpleInputComponent(); };
	...
	// GameLib::Actor player(new GameLib::SimpleInputComponent(), new GameLib::DungeonActorComponent(),
	//					  new GameLib::SimplePhysicsComponent(), new GameLib::DebugGraphicsComponent());
	...
	while (!context.quitRequested) {
		...
		gothicfont.draw((int)graphics.getWidth(), 0, "Hello, world!", GameLib::Blue,
